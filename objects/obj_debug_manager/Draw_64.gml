...
for (var i = 0; i < _count; i++) {
    ...
    with (_fighters[i]) {
        ...
        if !is_cpu 
        { draw_text_transformed_color(_hud_x + 35, _hud_y + 5, $"{obj_debug_manager.p1_stocks_left}", 0.7, 0.7, 0, c_fuchsia, c_fuchsia, c_purple, c_purple, 1) }
        else {
		if (variable_instance_exists(id, "stocks_left"))
		{
        draw_text_transformed_color(_hud_x + 35,_hud_y + 5,string(stocks_left),0.7,0.7,0,c_fuchsia,c_fuchsia,c_purple,c_purple,1);
		}
		}
